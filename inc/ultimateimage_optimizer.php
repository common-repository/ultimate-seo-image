...
class ultimateimage_optimizer
{
	...
	public function __construct( $settings=array() )
	{
		...
		// settings filter
		$this->settings = apply_filters('ultimateimage_optimizer_settings', $this->settings);
		...
		if( class_exists('DOMDocument') && false !== $this->settings['create_domdocument'] ) {
			...
			$this->document = apply_filters(
				'ultimateimage_optimizer_domdocument', // filter name
	 ...
	 * @return bool
	 */
	public function usiultimate_is_optimization_allowed( $data=array() )
	{
		if(
			...
			return apply_filters( 'usiultimate_is_optimization_allowed', false, $data );
		}

		return apply_filters( 'usiultimate_is_optimization_allowed', true, $data );
	}
	...
	{
		$usiultimate_is_optimization_allowed = $this->usiultimate_is_optimization_allowed([
			'post_id'       => get_the_ID(),
			'post_type'     => get_post_type(),
			'content'       => $content
		]);

		// filter for $usiultimate_is_optimization_allowed
		$usiultimate_is_optimization_allowed = apply_filters( 'optimize_html__usiultimate_is_optimization_allowed', $usiultimate_is_optimization_allowed );

		// Check if content could be optimized and if optimization is allowed
		if( false === $usiultimate_is_optimization_allowed && true !== $force_optimization ) {
			return $content;
		}
		...
		if( $imgTags->length ) {
            ...
            foreach ($imgTags as $tag) {
                ...
                if( isset($this->settings['override_alt']) ) {
                    ...
                    $alt = apply_filters('uiseo-alt', $alt);
                ...
                if( isset($this->settings['override_title']) ) {
                    ...
                    $title = apply_filters('uiseo-title', $title);
        ...
        if( isset($this->settings['link_title']) && $this->settings['link_title'] && $aTags->length ) {
	...
	public function optimize_image_attributes( $attr, $attachment = null )
	{
        ...
        if( function_exists('is_woocommerce') && is_woocommerce() && isset($this->settings['wc_title']) && $this->settings['wc_title']  ) {
            $attr['alt']    = get_the_title();
            $attr['title']  = get_the_title();
        }

		$attr['alt']    = apply_filters('uiseo-alt', $attr['alt']);
		$attr['title']  = apply_filters('uiseo-title', $attr['title']);
